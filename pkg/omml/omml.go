// Package omml 提供 LaTeX 到 Office Math Markup Language 的简化转换。
package omml
...
func parseLatex(latex string) []interface{} {
	...
	for i < len(latex) {
		...
		// 检查带上下标的 LaTeX 命令，如 \sum_{i=1}^{n}、\int_0^1
		if match := mathCmdSubSupPattern.FindStringSubmatch(remaining); match != nil {
			base := convertLaTeXCommand(match[1])
			sub := match[2]
			if sub == "" {
				sub = match[3]
			}
		...
		// 检查上下标组合
		if match := mathSubSupPattern.FindStringSubmatch(remaining); match != nil {
			base := match[1]
			sub := match[2]
			if sub == "" {
				sub = match[3]
		...
		// 检查分数
		if match := mathFracPattern.FindStringSubmatch(remaining); match != nil {
			num := match[1]
			den := match[2]
		...
		// 检查根号
		if match := mathSqrtPattern.FindStringSubmatch(remaining); match != nil {
		...
		// 检查带上标的 LaTeX 命令
		if match := mathCmdSupPattern.FindStringSubmatch(remaining); match != nil {
			base := convertLaTeXCommand(match[1])
			sup := match[2]
			if sup == "" {
				sup = match[3]
		...
		// 检查下标
		if match := mathSubPattern.FindStringSubmatch(remaining); match != nil {
			base := match[1]
			sub := match[2]
			if sub == "" {
				sub = match[3]
		...
		// 检查带下标的 LaTeX 命令
		if match := mathCmdSubPattern.FindStringSubmatch(remaining); match != nil {
			base := convertLaTeXCommand(match[1])
			sub := match[2]
			if sub == "" {
				sub = match[3]
		...
		// 检查普通文本
		if match := mathTextPattern.FindString(remaining); match != "" {
