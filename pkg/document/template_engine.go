...
func (tr *TemplateRenderer) analyzeParagraph(para *Paragraph, analysis *TemplateAnalysis) {
	tr.extractTemplateVariables(joinRunsText(para.Runs), analysis)
}
	...
	for rowIndex, row := range table.Rows {
		...
		for _, cell := range row.Cells {
			for _, para := range cell.Paragraphs {
				...
				// 检查循环语法
				if matches := templateEachStartPattern.FindStringSubmatch(fullText); len(matches) > 1 {
				...
				// 提取变量
				varMatches := templateVarPattern.FindAllStringSubmatch(fullText, -1)
...
func (tr *TemplateRenderer) extractTemplateVariables(text string, analysis *TemplateAnalysis) {
	// 变量: {{变量名}}
	varMatches := templateVarPattern.FindAllStringSubmatch(text, -1)
	for _, match := range varMatches {
		if len(match) >= 2 {
	...
	// 条件: {{#if 条件}}
	ifMatches := templateIfCountPattern.FindAllStringSubmatch(text, -1)
	...
	// 循环: {{#each 列表}}
	eachMatches := templateEachStartPattern.FindAllStringSubmatch(text, -1)
