...
func (d *Document) getNumberingManager() *NumberingManager {
	if d.numberingManager == nil {
		d.numberingManager = newNumberingManager()
	}
	return d.numberingManager
}

func (nm *NumberingManager) clone() *NumberingManager {
	if nm == nil {
		return newNumberingManager()
	}

	cloned := newNumberingManager()
	cloned.nextAbstractNumID = nm.nextAbstractNumID
	cloned.nextNumID = nm.nextNumID

	for key, abstractNum := range nm.abstractNums {
		cloned.abstractNums[key] = cloneAbstractNum(abstractNum)
	}
	for key, instance := range nm.numInstances {
		cloned.numInstances[key] = cloneNumInstance(instance)
	}

	return cloned
}

func cloneAbstractNum(source *AbstractNum) *AbstractNum {
	if source == nil {
		return nil
	}

	cloned := &AbstractNum{
		AbstractNumID: source.AbstractNumID,
		Levels:        make([]*Level, 0, len(source.Levels)),
	}
	for _, level := range source.Levels {
		cloned.Levels = append(cloned.Levels, cloneLevel(level))
	}
	return cloned
}

func cloneLevel(source *Level) *Level {
	if source == nil {
		return nil
	}
	...
	if source.NumFmt != nil {
		numFmt := *source.NumFmt
		cloned.NumFmt = &numFmt
	}
	if source.LevelText != nil {
		levelText := *source.LevelText
		cloned.LevelText = &levelText
	}
	if source.LevelJc != nil {
		levelJc := *source.LevelJc
		cloned.LevelJc = &levelJc
	}
	if source.PPr != nil {
		ppr := *source.PPr
		if source.PPr.Ind != nil {
			ind := *source.PPr.Ind
			ppr.Ind = &ind
		}
		cloned.PPr = &ppr
	...
	if source.RPr != nil {
		rpr := *source.RPr
		if source.RPr.FontFamily != nil {
			font := *source.RPr.FontFamily
			rpr.FontFamily = &font
		}
		cloned.RPr = &rpr
	}
...
func cloneNumInstance(source *NumInstance) *NumInstance {
	if source == nil {
		return nil
	}

	cloned := *source
	if source.AbstractNumID != nil {
		ref := *source.AbstractNumID
		cloned.AbstractNumID = &ref
	}
	return &cloned
}
...
func (d *Document) updateNumberingFile() {
	...
	// 添加所有抽象编号
	abstractKeys := make([]string, 0, len(manager.abstractNums))
	for key := range manager.abstractNums {
		abstractKeys = append(abstractKeys, key)
	}
	sort.Strings(abstractKeys)
	for _, key := range abstractKeys {
		numbering.AbstractNums = append(numbering.AbstractNums, manager.abstractNums[key])
	}

	// 添加所有编号实例
	numIDs := make([]string, 0, len(manager.numInstances))
	for key := range manager.numInstances {
		numIDs = append(numIDs, key)
	}
	sort.Strings(numIDs)
	for _, key := range numIDs {
		numbering.NumberingInstances = append(numbering.NumberingInstances, manager.numInstances[key])
	}
...
func (d *Document) loadNumberingManager() error {
	manager := newNumberingManager()

	data, exists := d.parts["word/numbering.xml"]
	if !exists {
		d.numberingManager = manager
		return nil
	}
	...
	for _, abstractNum := range numbering.AbstractNums {
		...
		manager.abstractNums["loaded:"+abstractNum.AbstractNumID] = cloneAbstractNum(abstractNum)
		if id, err := strconv.Atoi(abstractNum.AbstractNumID); err == nil && id >= manager.nextAbstractNumID {
			manager.nextAbstractNumID = id + 1
		}
	}

	for _, numInstance := range numbering.NumberingInstances {
		if numInstance == nil {
			continue
		}
		manager.numInstances[numInstance.NumID] = cloneNumInstance(numInstance)
		if id, err := strconv.Atoi(numInstance.NumID); err == nil && id >= manager.nextNumID {
			manager.nextNumID = id + 1
		}
	}
