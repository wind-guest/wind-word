...
func (d *Document) AddHeadingWithBookmark(text string, level int, bookmarkName string) *Paragraph {
	...
	paragraph.BookmarkStart = bookmark
	paragraph.BookmarkEnd = &BookmarkEnd{ID: bookmarkID}
		...
		if paragraph, ok := element.(*Paragraph); ok {
			...
			if level > 0 && level <= maxLevel {
				...
				if text != "" {
					...
					paragraph.BookmarkStart = &BookmarkStart{
						ID:   fmt.Sprintf("%d", entryIndex),
						Name: anchor,
					}
					paragraph.BookmarkEnd = &BookmarkEnd{
						ID: fmt.Sprintf("%d", entryIndex),
					}

					newElements = append(newElements, paragraph)
