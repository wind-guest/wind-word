...
type Document struct {
	...
	nextImageID int
	// 列表编号管理器，按文档隔离，避免跨文档污染
	numberingManager *NumberingManager
	// 脚注/尾注管理器，按文档隔离，避免跨文档污染
	footnoteManager *FootnoteManager
}

const (
	maxDocxInputSize = 512 << 20
	maxDocxPartSize  = 128 << 20
...
type Paragraph struct {
	...
	BookmarkStart *BookmarkStart       `xml:"-"`
	BookmarkEnd   *BookmarkEnd         `xml:"-"`
	Runs          []Run                `xml:"w:r"`
	Math          *omml.OfficeMath     `xml:"m:oMath,omitempty"`
	MathPara      *omml.OfficeMathPara `xml:"m:oMathPara,omitempty"`
}

// MarshalXML 自定义Paragraph的XML序列化，确保段落内书签顺序正确。
func (p *Paragraph) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	if p.BookmarkStart != nil {
		if err := e.Encode(p.BookmarkStart); err != nil {
			return err
		}
	}
	...
	if p.MathPara != nil {
		if err := e.Encode(p.MathPara); err != nil {
			return err
		}
	}

	if p.Math != nil {
		if err := e.Encode(p.Math); err != nil {
			return err
		}
	}

	if p.BookmarkEnd != nil {
		if err := e.Encode(p.BookmarkEnd); err != nil {
			return err
		}
	}
...
func (r *Run) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	// 序列化行内公式（如果存在）
	if r.Math != nil {
		if err := e.Encode(r.Math); err != nil {
			return err
		}
	}
...
func New() *Document {
	...
	doc := &Document{
		...
		numberingManager: newNumberingManager(),
		footnoteManager:  newFootnoteManager(),
		documentRelationships: &Relationships{
...
func OpenFromMemory(readCloser io.ReadCloser) (*Document, error) {
	if readCloser == nil {
		return nil, WrapError("open_file", ErrInvalidDocument)
	}

	defer readCloser.Close()
	Infof("正在打开文档")

	fileData, err := readAllWithLimit(readCloser, maxDocxInputSize)
	if err != nil {
		return nil, WrapError("read_file", err)
	}
...
func openFromZipReader(zipReader *zip.Reader, filename string) (*Document, error) {
	...
	if len(zipReader.File) > maxDocxEntries {
		return nil, WrapErrorWithContext("open_file", fmt.Errorf("zip entry count exceeds limit (%d)", maxDocxEntries), filename)
	}
	...
	for _, file := range zipReader.File {
		data, err := readZipPart(file, &totalSize)
		if err != nil {
	...
	if err := doc.loadNumberingManager(); err != nil {
		Debugf("解析编号定义失败，使用默认值: %v", err)
		doc.numberingManager = newNumberingManager()
	}
	if err := doc.loadFootnoteManager(); err != nil {
		Debugf("解析脚注尾注失败，使用默认值: %v", err)
...
func (d *Document) Save(filename string) (err error) {
	...
	defer func() {
		if closeErr := zipWriter.Close(); err == nil && closeErr != nil {
			err = WrapError("close_zip_writer", closeErr)
		}
		if closeErr := file.Close(); err == nil && closeErr != nil {
			err = WrapErrorWithContext("close_file", closeErr, filename)
		}
	}()

	if err = d.prepareForWrite(); err != nil {
		return err
	}

	if err = d.writePartsToZip(zipWriter); err != nil {
		return err
	}
...
func (d *Document) AddHeadingParagraphWithBookmark(text string, level int, bookmarkName string) *Paragraph {
	...
	// 创建段落的Run列表
	runs := make([]Run, 0)
	var bookmarkStart *BookmarkStart
	var bookmarkEnd *BookmarkEnd

	// 如果需要添加书签，在段落开始处添加书签开始标记
	if bookmarkName != "" {
		bookmarkID := fmt.Sprintf("bookmark_%d_%s", len(d.Body.Elements), bookmarkName)
		bookmarkStart = &BookmarkStart{ID: bookmarkID, Name: bookmarkName}
		bookmarkEnd = &BookmarkEnd{ID: bookmarkID}
...
func (d *Document) parseParagraph(decoder *xml.Decoder, startElement xml.StartElement) (*Paragraph, error) {
	...
	for {
		...
		case xml.StartElement:
			...
			case "oMath":
				math, err := omml.ParseOfficeMath(decoder, t)
				if err != nil {
					return nil, err
				}
				paragraph.Math = math
			case "oMathPara":
				mathPara, err := omml.ParseOfficeMathPara(decoder, t)
				if err != nil {
					return nil, err
				}
				paragraph.MathPara = mathPara
			default:
				// 跳过其他元素
...
func readZipPart(file *zip.File, totalSize *uint64) ([]byte, error) {
	if file.UncompressedSize64 > maxDocxPartSize {
		return nil, fmt.Errorf("zip entry %q exceeds %d bytes", file.Name, maxDocxPartSize)
	}

	nextTotal := *totalSize + file.UncompressedSize64
	if nextTotal > maxDocxInputSize {
		return nil, fmt.Errorf("docx payload exceeds %d bytes", maxDocxInputSize)
	}
	...
	data, err := readAllWithLimit(rc, maxDocxPartSize)
	if err != nil {
		return nil, err
	}

	*totalSize += uint64(len(data))
	if *totalSize > maxDocxInputSize {
		return nil, fmt.Errorf("docx payload exceeds %d bytes", maxDocxInputSize)
	}
...
func (d *Document) prepareForWrite() error {
...
func (d *Document) writePartsToZip(zipWriter *zip.Writer) error {
	partNames := make([]string, 0, len(d.parts))
