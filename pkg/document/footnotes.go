...
func (d *Document) getFootnoteManager() *FootnoteManager {
	if d.footnoteManager == nil {
		d.footnoteManager = newFootnoteManager()
	}
...
func (d *Document) updateFootnotesFile() {
	...
	// 添加所有脚注
	footnoteIDs := make([]string, 0, len(manager.footnotes))
	for id := range manager.footnotes {
		footnoteIDs = append(footnoteIDs, id)
	}
	sort.Slice(footnoteIDs, func(i, j int) bool {
		left, leftErr := strconv.Atoi(footnoteIDs[i])
		right, rightErr := strconv.Atoi(footnoteIDs[j])
		if leftErr == nil && rightErr == nil {
			return left < right
		}
		return footnoteIDs[i] < footnoteIDs[j]
	})
	for _, id := range footnoteIDs {
		footnotes.Footnotes = append(footnotes.Footnotes, manager.footnotes[id])
	}
...
func (d *Document) updateEndnotesFile() {
	...
	// 添加所有尾注
	endnoteIDs := make([]string, 0, len(manager.endnotes))
	for id := range manager.endnotes {
		endnoteIDs = append(endnoteIDs, id)
	}
	sort.Slice(endnoteIDs, func(i, j int) bool {
		left, leftErr := strconv.Atoi(endnoteIDs[i])
		right, rightErr := strconv.Atoi(endnoteIDs[j])
		if leftErr == nil && rightErr == nil {
			return left < right
		}
		return endnoteIDs[i] < endnoteIDs[j]
	})
	for _, id := range endnoteIDs {
		endnotes.Endnotes = append(endnotes.Endnotes, manager.endnotes[id])
	}
...
func (d *Document) loadFootnoteManager() error {
	manager := newFootnoteManager()

	if data, exists := d.parts["word/footnotes.xml"]; exists {
		var footnotes Footnotes
		if err := xml.Unmarshal(data, &footnotes); err != nil {
			return err
		}
		for _, footnote := range footnotes.Footnotes {
			if footnote == nil || footnote.ID == "" || footnote.ID == "-1" {
				continue
			}
			manager.footnotes[footnote.ID] = footnote
			if id, err := strconv.Atoi(footnote.ID); err == nil && id >= manager.nextFootnoteID {
				manager.nextFootnoteID = id + 1
			}
		}
	}

	if data, exists := d.parts["word/endnotes.xml"]; exists {
		var endnotes Endnotes
		if err := xml.Unmarshal(data, &endnotes); err != nil {
			return err
		}
		for _, endnote := range endnotes.Endnotes {
			if endnote == nil || endnote.ID == "" || endnote.ID == "-1" {
				continue
			}
			manager.endnotes[endnote.ID] = endnote
			if id, err := strconv.Atoi(endnote.ID); err == nil && id >= manager.nextEndnoteID {
				manager.nextEndnoteID = id + 1
			}
		}
	}

	d.footnoteManager = manager
	return nil
}
