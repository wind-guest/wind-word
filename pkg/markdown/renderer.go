...
var (
	markdownWhitespacePattern = regexp.MustCompile(`\s+`)
	latexDisplayFracPattern   = regexp.MustCompile(`\\frac\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}`)
	latexDisplaySqrtPattern   = regexp.MustCompile(`\\sqrt\s*(?:\[([^\]]*)\])?\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}`)
	latexSupBracePattern      = regexp.MustCompile(`\^\{([^{}]*)\}`)
	latexSupSimplePattern     = regexp.MustCompile(`\^([a-zA-Z0-9])`)
	latexSubBracePattern      = regexp.MustCompile(`_\{([^{}]*)\}`)
	latexSubSimplePattern     = regexp.MustCompile(`_([a-zA-Z0-9])`)
)
...
func (r *WordRenderer) cleanText(text string) string {
	// 移除多余的空白字符
	text = markdownWhitespacePattern.ReplaceAllString(text, " ")
...
func (r *WordRenderer) renderMathBlock(node ast.Node) (ast.WalkStatus, error) {
	...
	para := r.doc.AddMathFormula(latex, true)
	para.Properties = &document.ParagraphProperties{
...
func convertLaTeXToDisplay(latex string) string {
	...
	for latexDisplayFracPattern.MatchString(latex) {
		latex = latexDisplayFracPattern.ReplaceAllStringFunc(latex, func(match string) string {
			parts := latexDisplayFracPattern.FindStringSubmatch(match)
			if len(parts) == 3 {
	...
	for latexDisplaySqrtPattern.MatchString(latex) {
		latex = latexDisplaySqrtPattern.ReplaceAllStringFunc(latex, func(match string) string {
			parts := latexDisplaySqrtPattern.FindStringSubmatch(match)
			if len(parts) == 3 {
				deg := parts[1]
	...
	latex = latexSupBracePattern.ReplaceAllStringFunc(latex, func(match string) string {
		parts := latexSupBracePattern.FindStringSubmatch(match)
		if len(parts) == 2 {
			return convertToSuperscript(parts[1])
		}
		return match
	})
	latex = latexSupSimplePattern.ReplaceAllStringFunc(latex, func(match string) string {
		parts := latexSupSimplePattern.FindStringSubmatch(match)
		if len(parts) == 2 {
			return convertToSuperscript(parts[1])
		}
		return match
	...
	latex = latexSubBracePattern.ReplaceAllStringFunc(latex, func(match string) string {
		parts := latexSubBracePattern.FindStringSubmatch(match)
		if len(parts) == 2 {
			return convertToSubscript(parts[1])
		}
		return match
	})
	latex = latexSubSimplePattern.ReplaceAllStringFunc(latex, func(match string) string {
		parts := latexSubSimplePattern.FindStringSubmatch(match)
		if len(parts) == 2 {
			return convertToSubscript(parts[1])
		}
		return match
