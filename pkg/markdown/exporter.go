...
func NewExporter(opts *ExportOptions) *Exporter {
	...
	return &Exporter{opts: opts.clone()}
}
...
func (e *Exporter) ExportToFile(docxPath, mdPath string, options *ExportOptions) error {
	...
	// 设置图片输出路径
	resolved := e.opts.clone()
	if options != nil {
		resolved = options.clone()
	}
	if resolved.ExtractImages && resolved.ImageOutputDir == "" {
		resolved.ImageOutputDir = filepath.Dir(mdPath)
	}

	// 转换为Markdown
	markdown, err := e.ExportToString(doc, resolved)
	if err != nil {
		return err
	}
...
func (o *ExportOptions) clone() *ExportOptions {
	...
	cloned := *o
	if o.CustomStyleMap != nil {
		cloned.CustomStyleMap = make(map[string]string, len(o.CustomStyleMap))
		for key, value := range o.CustomStyleMap {
			cloned.CustomStyleMap[key] = value
		}
	}
