...
func (w *MarkdownWriter) Write() ([]byte, error) {
	if w.doc == nil {
		return nil, NewExportError("InvalidDocument", "document is nil", ErrInvalidDocument)
	}
	if w.opts == nil {
		w.opts = DefaultExportOptions()
	}
	...
	// 保持文档元素原始顺序，避免段落和表格重排
	if w.doc.Body != nil {
		for _, element := range w.doc.Body.Elements {
			var err error
			switch elem := element.(type) {
			case *document.Paragraph:
				err = w.writeParagraph(elem)
			case *document.MathParagraph:
				err = w.writeMathParagraph(elem)
			case *document.Table:
				err = w.writeTable(elem)
			default:
				continue
			}
...
func (w *MarkdownWriter) writeParagraph(para *document.Paragraph) error {
	...
	if para.MathPara != nil {
		return w.writeBlockMath(omml.OMMLParaToLaTeX(para.MathPara))
	}
...
func (w *MarkdownWriter) writeMathParagraph(para *document.MathParagraph) error {
	if para == nil {
		return nil
	}

	prefix := w.extractRunsText(para.Runs)
	if para.MathPara != nil {
	...
	if para.Math != nil {
		...
		line += "$" + omml.OMMLToLaTeX(para.Math) + "$"
...
func (w *MarkdownWriter) writeBlockMath(latex string) error {
...
func (w *MarkdownWriter) extractParagraphText(para *document.Paragraph) string {
	...
	text := w.extractRunsText(para.Runs)
	if para.Math != nil {
		text += "$" + omml.OMMLToLaTeX(para.Math) + "$"
	}
	if para.MathPara != nil {
		text += "$" + omml.OMMLParaToLaTeX(para.MathPara) + "$"
	}

	return text
}

func (w *MarkdownWriter) extractRunsText(runs []document.Run) string {
	var result strings.Builder

	for i := range runs {
		result.WriteString(w.formatRunText(&runs[i]))
	}
...
func (w *MarkdownWriter) formatRunText(run *document.Run) string {
	...
	if run.Math != nil {
		result.WriteString("$" + omml.OMMLToLaTeX(run.Math) + "$")
	}
...
func (w *MarkdownWriter) getHeadingLevel(style string) int {
	// 提取数字
	matches := headingLevelPattern.FindString(style)
	if matches != "" {
